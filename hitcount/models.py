...
class HitManager(models.Manager):
    ...
    def filter_active(self, *args, **kwargs):
        ...
        return within the last seven days..  After that the hit
        ...
        '''
        grace = getattr(settings, 'HITCOUNT_KEEP_HIT_ACTIVE', {'days':1})
...
class Hit(models.Model):
    ...
    # For tracking orgsub user activity so we can provide stats    
    orgsub =  models.ForeignKey(Organization, null=True, editable=False)
    orgsub_ip = models.BooleanField(editable=False)
    orgsub_member = models.BooleanField(editable=False)
